...
- `CameraManager.OpenAsync` / `CloseAsync` 当前仅为接入位置，会明确记录尚未接入真实 SDK。目前已根据 RobotCamX 头文件编写 P/Invoke，等待 `LibRobotCamX.so` 到位；相机管理流程暂不录像、不识别。
...
配置里的相机 IP 和账号仍为示例，接入真实 SDK 前需要补全。旧的原生 SDK 测试 Controller 已移除，相机启停由控制中心调度；底层声明集中在 `NativeInterop/NativeMethods.cs`，业务直接调用 `NativeMethods.RobotX_*`；结构体、回调及调用示例见 `Native/README.md`。
